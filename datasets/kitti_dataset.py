...
class SemKITTI(data.Dataset):
    def __init__(self, config,color_jitter = None, imageset='train'):
        ...
        for ii,i_folder in enumerate(split):
            ...
            rgb_path_2 = os.path.join(config.data_path, str(i_folder).zfill(2),'image_2')
            rgb_path_3 = os.path.join(config.data_path, str(i_folder).zfill(2),'image_3')
            depth_path_2 = os.path.join(config.data_path, str(i_folder).zfill(2),'depth2')
            files = list(pathlib.Path(voxel_path).glob('*.bin')) #.label
            for filename in files:
                frame_id = os.path.splitext(os.path.basename(filename))[0]
                rgb_name_2 = os.path.join(rgb_path_2,frame_id + ".png")
                rgb_name_3 = os.path.join(rgb_path_3,frame_id + ".png")
                depth_name_2 = os.path.join(depth_path_2,frame_id + ".npy")
                voxel_name = os.path.join(voxel_path,frame_id + ".label")
                scan = {
                        "frame_id":frame_id,
    ...
    def proj_box(self,P2,P3,T_velo_2_cam):
        box_xyxys, fov_masks,proj_uvds,proj_uvds3 = list(),list(),list(),list()
        ...
        return box_xyxys, fov_masks,proj_uvds,proj_uvds3
    ...
    def __getitem__(self, index):
        ...
        rgb3 = self.getrgb(scan["rgb_path_3"])
        depth2 = self.getdepth(scan["depth_path_2"])
        rgb = np.concatenate((rgb2,rgb3), axis=0)
        voxel_label = self.getvox(scan["voxel_path"])
        frame_id = scan["frame_id"]
        box_xyxys, fov_masks,proj_uvds,proj_uvds3 = self.projected_assets[scan["asset_idx"]]
        data = {
            "frame_id": frame_id,
